...
def _prompt(u, text: str) -> tuple[str, str, dict, str]:
    ...
    if k == "story":
        ...
        usr = (
            ...
            "  related_tags: 資料中一起出現、關係密切的其他標籤,必須連同 # 照抄(最多 6 個)\n\n"
    ...
    if k in ("week", "month"):
        ...
        if k == "month":
            ...
            # 兩種來源:近期的週有 AI 週報摘要;較舊的週只有系統算的統計摘要
            # (標示「本週為統計摘要,無逐則內容」)。後者不可以當成看過原文。
            src = ("=== 資料:本月各週的整理 ===\n"
                   "(標示「統計摘要」的週沒有逐則內容,只能依據其標籤統計推斷當週主軸,"
                   "不得編造具體事件或聲稱看過該週訊息)\n")
        else:
        ...
        usr = (
            ...
            "detail(說明,120 字內)、"
            "tags(這個主軸涉及的標籤,**必須連同 # 一起原封不動照抄**資料中出現過的標籤,"
            "例如 #記憶體;每個主軸盡量給 2～5 個)、"
    ...
    if k == "clusters":
        ...
        usr = (
            "請把下方標籤歸納成 10～18 個上層主題,輸出 JSON:\n"
            "  groups: 每組 name(主題名稱,20 字內)、blurb(這組的共同性,60 字內)、"
            "tags(屬於這組的標籤)\n\n"
            "規則:\n"
            "  - tags 內的每個標籤都必須原封不動來自下方清單,不可發明、不可改寫\n"
            "  - 一個標籤最多只能屬於一組\n"
            "  - **盡可能把多數標籤都歸類進去,未歸類的越少越好**。\n"
            "    清單裡大量的個股名稱請歸進它所屬的產業主題(例如某記憶體廠歸到記憶體/半導體),\n"
            "    不要因為它出現次數少就略過。真的看不出所屬領域的才留著不歸類。\n"
            "  - 若某個產業的個股很多,可以再細分成兩三個主題,不必硬擠成一組\n"
            "  - 不要輸出任何次數或數字\n"
