...
# 不做敘事線的標籤。
#
# 判準不是「重不重要」,而是**它是「主題」還是「格式/標記」**:
# 敘事線的前提是「同一個主體隨時間演進」,但這些標籤的主體並不連續 ——
...
#   #阿非電台  節目時段標記(「1430》…」「公告》今日無法 CB LIVE」),不是主題
# 硬要對它們寫因果時間線,會逼模型**編造連續性** —— 這種幻覺驗證器擋不到
# (驗證器擋的是假數字/假 id/假標籤,擋不了假敘事),所以必須在源頭排除。
# 它們仍會正常出現在排行、週月報、主題地圖與雷達裡,只是不做敘事線。
STORY_SKIP_TAGS = {
    t.strip().lower()
    for t in os.getenv(
        "STORY_SKIP_TAGS",
        "#重要,#ai整理,#盤點,#焦點,#法人,#盤勢,#新高,#AI主動式ETF整理,#阿非電台",
    ).split(",")
    if t.strip()
}
